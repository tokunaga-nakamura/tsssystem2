...
{
    public partial class frm_uriage_syukei : Form
    {
        TssSystemLibrary tss = new TssSystemLibrary();
        DataTable dt_m = new DataTable();
        DataTable w_dt_insatu = new DataTable();
        
        public frm_uriage_syukei()
        {
            InitializeComponent();
        }
        ...
        private void btn_kensaku_Click(object sender, EventArgs e)
        {
            ...
            label3.Text = tb_uriage_date.Text;
        }
        ...
        private void kensaku2()
        {
            ...
            if (tb_uriage_date.Text == "")
            {
                MessageBox.Show("売上計上日を指定してください。");
                return;
            }
            ...
            int sql_cnt = 0;
            string str_yyyymm = tb_uriage_date.Text.Substring(0,7);　//yyyymmdd
            DateTime YYYYMM = new DateTime();

            YYYYMM = DateTime.Parse(str_yyyymm.ToString());

            //入力した指定月の月初と月末の日付を取得
            DateTime tougetu_syo = new DateTime(YYYYMM.Year, YYYYMM.Month, 1);
            DateTime tougetu_matu = new DateTime(YYYYMM.Year, YYYYMM.Month, 1).AddMonths(1).AddDays(-1);


            //入力した指定月の前月の年月を取得
            string zengetu_yyyymm = YYYYMM.AddMonths(-1).ToString("yyyy/MM");
            ...
            sql = sql + " and uriage_date <= '" + tb_uriage_date.Text.ToString() + "' ";
            ...
            else
            {
                ...
                if (tb_torihikisaki_cd.Text == "") //取引先コード指定しない場合
                {
                    ...
                    dt_kensaku2 = tss.OracleSelect("select max(torihikisaki_cd),max(torihikisaki_name),max(uriage_simebi) from tss_uriage_m where uriage_simebi between '" + tougetu_syo.ToShortDateString() + "' and '" + tougetu_matu.ToShortDateString() + "' and uriage_date <= '" + tb_uriage_date.Text.ToString() + "' group by  torihikisaki_cd order by max(uriage_simebi),max(torihikisaki_cd)");

                    //dt_kensaku3 = tss.OracleSelect("select sum(uriage_yotei_1),sum(uriage_yotei_2) from tss_uriage_yotei_m where uriage_yotei_nengetu = '" + str_yyyymm.ToString() + "'");
                ...
                else　//取引先コード指定する場合
                {
                    ...
                    //dt_kensaku3 = tss.OracleSelect("select sum(uriage_yotei_1),sum(uriage_yotei_2) from tss_uriage_yotei_m where torihikisaki_cd = '" + tb_torihikisaki_cd.Text.ToString() + "' and  uriage_yotei_nengetu = '" + str_yyyymm.ToString() + "'");
                ...
                for (int i = 0; i <= rc2 - 1; i++)
                {
                    dt_uriage_yotei = tss.OracleSelect("select sum(uriage_yotei_1),sum(uriage_yotei_2) from tss_uriage_yotei_m where torihikisaki_cd = '" + dt_kensaku2.Rows[i][0].ToString() + "' and  uriage_yotei_nengetu = '" + str_yyyymm.ToString() + "'");
                ...
                object obj_gessyo_yotei_ttl = dt_kensaku2.Compute("Sum(YOTEI_KINGAKU_1)", null);
                object obj_tyuukan_yotei_ttl = dt_kensaku2.Compute("Sum(YOTEI_KINGAKU_2)", null);



                if (obj_gessyo_yotei_ttl.ToString() == "")
                {
                    tb_gessyo_yotei.Text = "0";
                }
                else
                {
                    tb_gessyo_yotei.Text = decimal.Parse(obj_gessyo_yotei_ttl.ToString()).ToString();
                }


                if (obj_tyuukan_yotei_ttl.ToString() == "")
                {
                    tb_tyuukan_yotei.Text = "0";
                }
                else
                {
                    tb_tyuukan_yotei.Text = decimal.Parse(obj_tyuukan_yotei_ttl.ToString()).ToString();
                }



                list_disp3(dt_kensaku2);

                tb_uriage_ruikei.Text = obj_uriage_ttl.ToString();
                tb_kouchin_ruikei.Text = obj_kouchin_ttl.ToString();
                tb_hukusizai_ruikei.Text = obj_hukusizai_ttl.ToString();
                tb_buhin_ruikei.Text = obj_buhin_ttl.ToString();
                tb_kouchin_hukusizai_ruikei.Text = dc_kouchin_hukusizai_ttl.ToString();
                ...
                {
                    string tasseiritu1 = (decimal.Parse(tb_kouchin_hukusizai_ruikei.Text.ToString()) / decimal.Parse(tb_gessyo_yotei.Text.ToString())).ToString("P");
                ...
                {
                    string tasseiritu2 = (decimal.Parse(tb_kouchin_hukusizai_ruikei.Text.ToString()) / decimal.Parse(tb_tyuukan_yotei.Text.ToString())).ToString("P");
                ...
                //集計後、カンマ区切り数にする
                decimal number = decimal.Parse(tb_uriage_ruikei.Text.ToString()); // 変換前の数値
                string str = String.Format("{0:#,0}", number); // 変換後
                tb_uriage_ruikei.Text = str;

                number = decimal.Parse(tb_kouchin_ruikei.Text.ToString()); // 変換前の数値
                str = String.Format("{0:#,0}", number); // 変換後
                tb_kouchin_ruikei.Text = str;

                number = decimal.Parse(tb_hukusizai_ruikei.Text.ToString()); // 変換前の数値
                str = String.Format("{0:#,0}", number); // 変換後
                tb_hukusizai_ruikei.Text = str;

                number = decimal.Parse(tb_buhin_ruikei.Text.ToString()); // 変換前の数値
                str = String.Format("{0:#,0}", number); // 変換後
                tb_buhin_ruikei.Text = str;

                number = decimal.Parse(tb_kouchin_hukusizai_ruikei.Text.ToString()); // 変換前の数値
                str = String.Format("{0:#,0}", number); // 変換後
                tb_kouchin_hukusizai_ruikei.Text = str;
        ...
        private void list_disp3(DataTable in_dt)
        {
            //リードオンリーにする
            dgv_ruikei.ReadOnly = true;
            //行ヘッダーを非表示にする
            dgv_ruikei.RowHeadersVisible = false;
            //カラム幅の自動調整（ヘッダーとセルの両方の最長幅に調整する）
            dgv_ruikei.AutoSizeColumnsMode = DataGridViewAutoSizeColumnsMode.AllCells;
            //セルの高さ変更不可
            dgv_ruikei.AllowUserToResizeRows = false;
            //カラムヘッダーの高さ変更不可
            dgv_ruikei.ColumnHeadersHeightSizeMode = DataGridViewColumnHeadersHeightSizeMode.DisableResizing;
            //削除不可にする（コードからは削除可）
            dgv_ruikei.AllowUserToDeleteRows = false;
            //１行のみ選択可能（複数行の選択不可）
            dgv_ruikei.MultiSelect = false;
            //セルを選択すると行全体が選択されるようにする
            dgv_ruikei.SelectionMode = DataGridViewSelectionMode.FullRowSelect;
            //DataGridView1にユーザーが新しい行を追加できないようにする
            dgv_ruikei.AllowUserToAddRows = false;

            dgv_ruikei.DataSource = null;
            dgv_ruikei.DataSource = in_dt;
            dt_m = in_dt;
            //DataGridViewのカラムヘッダーテキストを変更する
            dgv_ruikei.Columns[0].HeaderText = "取引先コード";
            dgv_ruikei.Columns[1].HeaderText = "取引先名";
            dgv_ruikei.Columns[2].HeaderText = "売上金額";
            dgv_ruikei.Columns[3].HeaderText = "工賃金額";
            dgv_ruikei.Columns[4].HeaderText = "副資材金額";
            dgv_ruikei.Columns[5].HeaderText = "部品金額";
            dgv_ruikei.Columns[6].HeaderText = "工賃+副資材金額";
            dgv_ruikei.Columns[7].HeaderText = "月初予定金額";
            dgv_ruikei.Columns[8].HeaderText = "中間予定金額";
            dgv_ruikei.Columns[9].HeaderText = "売上締日";


            dgv_ruikei.Columns[2].DefaultCellStyle.Format = "#,0";
            dgv_ruikei.Columns[3].DefaultCellStyle.Format = "#,0";
            dgv_ruikei.Columns[4].DefaultCellStyle.Format = "#,0";
            dgv_ruikei.Columns[5].DefaultCellStyle.Format = "#,0";
            dgv_ruikei.Columns[6].DefaultCellStyle.Format = "#,0";
            dgv_ruikei.Columns[7].DefaultCellStyle.Format = "#,0";
            dgv_ruikei.Columns[8].DefaultCellStyle.Format = "#,0";

            dgv_ruikei.Columns[2].DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleRight;
            dgv_ruikei.Columns[3].DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleRight;
            dgv_ruikei.Columns[4].DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleRight;
            dgv_ruikei.Columns[5].DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleRight;
            dgv_ruikei.Columns[6].DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleRight;
            dgv_ruikei.Columns[7].DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleRight;
            dgv_ruikei.Columns[8].DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleRight;

            dgv_ruikei.Columns[6].DefaultCellStyle.BackColor = Color.LightGray;
            dgv_ruikei.Columns[7].DefaultCellStyle.BackColor = Color.LightGray;
            dgv_ruikei.Columns[8].DefaultCellStyle.BackColor = Color.LightGray;
        ...
        private void frm_uriage_syukei_Load(object sender, EventArgs e)
        {
            tb_uriage_date.Focus();
        }

        private void frm_uriage_syukei_Activated(object sender, EventArgs e)
        {
            tb_uriage_date.Focus();
        }
    }
