...
namespace TSS_SYSTEM
{
    public partial class frm_seihin_tanka_m : Form
    {
                ...
                else
                {
                    ...
                    if (chk_seihin_tanka() == true)
                    {
                       ...
                       for (int i = 0; i < rc; i++)
                       {
                        ...
                        double goukei = double.Parse(dt_work.Compute("SUM(tanka)", null).ToString());
                        tb_tanka_goukei.Text = goukei.ToString("#,0.00");
        ...
        private void gamen_disp(DataTable in_dt_work)
        {
            ...
            DataTable dt_work = new DataTable();
            //dt_work = tss.OracleSelect("select t.tanka_kbn,s1.kubun_name 単価区分名,t.bumon_cd,s2.kubun_name 部門名,t.tanka,t.bikou from tss_seihin_tanka_m t LEFT OUTER JOIN TSS_KUBUN_M s1 ON t.KUBUN_CD = s1.KUBUN_CD LEFT OUTER JOIN TSS_KUBUN_M s2 ON t.GOKAN_BUHIN_CD = s2.BUHIN_CD WHERE seihin_cd = '" + str_w2.ToString() + "' and seihin_kousei_no = '" + str_w3.ToString() + "' ORDER BY t.SEQ");
...
　 
        ...
        private void dgv_m_CellEndEdit(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (e.ColumnIndex == 4)
            {
                ...
                double goukei = double.Parse(dt_w2.Compute("SUM(tanka)", null).ToString());
                tb_tanka_goukei.Text = goukei.ToString("#,0.00");
